...
export function SuggestControls({ 
  ...
  // Load the setting from localStorage, default to true (ON)
  const [useCameraForMood, setUseCameraForMood] = useState(() => {
    if (typeof window === 'undefined') return true; // Default to true during SSR
    const stored = localStorage.getItem('useCameraForMood');
  ...
  const handleToggleCamera = (checked: boolean) => {
    setUseCameraForMood(checked);
    if (typeof window !== 'undefined') {
      localStorage.setItem('useCameraForMood', String(checked));
    }
    console.log('🎥 Camera for mood:', checked ? 'ENABLED' : 'DISABLED');
  };
        ...
        <CardContent className="space-y-4">
          ...
          {mood && (
            <div className="p-3 bg-muted/50 rounded-lg space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {mood.timestamp && (
                  <>📅 Detected: {new Date(mood.timestamp).toLocaleTimeString()}</>
                )}
              </p>
            </div>
          )}
